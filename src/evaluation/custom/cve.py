...
class CounterfactualVisualExplainer:
    ...
    def explain(
        self, features_query, features_distractor, class_query, class_distractor
    ):
        ...
        for i in range(self.max_edits):
            ...
            features_query_edited_before = features_query_flat_edited.clone().view(
                1, C, H, W
            )
            features_query_edited_before = features_query_edited_before.flatten(1)
            outputs_query_edited_before = self.model.classifier(
                features_query_edited_before
            )
            # query_class_score_before_edit = outputs_query_edited_before[
            #     0, class_query
            # ].item()
            distractor_class_score_before_edit = outputs_query_edited_before[
                0, class_distractor
            ].item()
            ...
            for f_query in range(features_query_flat.shape[-1]):
                ...
                for f_distractor in range(features_distractor_flat.shape[-1]):
                    ...
                    features_query_flat_edited_tmp = (
                        features_query_flat_edited_tmp.flatten(1)
                    )
                    outputs_query_edited_tmp = self.model.classifier(
                        features_query_flat_edited_tmp
                    )
                    distractor_class_score_after_edit = outputs_query_edited_tmp[
                        0, class_distractor
                    ].item()

                    swap_value = (
                        distractor_class_score_after_edit
                        - distractor_class_score_before_edit
                    )
            ...
            features_query_flat_edited[:, best_swap_idxs[0]] = features_distractor_flat[
                :, best_swap_idxs[1]
            ]
            features_query_flat_edited_after = features_query_flat_edited.clone().view(
                1, C, H, W
            )
            features_query_flat_edited_after = features_query_flat_edited_after.flatten(
                1
            )
            outputs_query_edited_after = self.model.classifier(
                features_query_flat_edited_after
            )
...
def draw_bounding_boxes(image, centers, colors, stride=32):
    for i, center in enumerate(centers):
        image = cv2.rectangle(
            image,
            (center[0] - stride // 2, center[1] - stride // 2),
            (center[0] + stride // 2, center[1] + stride // 2),
            colors(i),
            1,
        )
...
parser.add_argument(
    "--data", metavar="DIR", required=True, help="path to dataset (default: funnybird)"
)
parser.add_argument(
    "--model", required=True, choices=["vgg16"], help="model architecture"
)
parser.add_argument(
    "--checkpoint_name",
    type=str,
...
parser.add_argument(
    ...
    action="store_true",
    help="query and distractor images have same orientation "
    "and only differ in one part",
)
parser.add_argument(
    "--different_ori_one_diff",
    default=False,
    action="store_true",
    help="query and distractor images have different orientation "
    "and only differ in one part",
)
...
def main():
    ...
    )  # BS of 2, one is query one is distractor
    ...
    nr_swaps_reasonable = (
        []
    )  # a swap is reasonable if it touches the same part in both images
    nr_different_parts = []
    itrs = 0
    while itrs < args.nr_itrs:
        for sample in test_dataloader:
            ...
            if args.same_ori_one_diff:
                ...
                params_distractor["light_pitch"] = params_query["light_pitch"]
                params_distractor["light_roll"] = params_query["light_roll"]
                image_distractor, _ = test_dataset.render_class(
                    class_distractor, params_distractor
                )
                image_distractor = image_distractor.to(device)
                image_distractor = image_distractor[:, :, 16:-16, 16:-16]
            ...
            swaps_i1, swaps_i2, class_edited = cve.explain(
                features_query, features_distractor, class_query, class_distractor
            )
            ...
            nr_different_parts.append(
                test_dataset.class_distance(class_query, class_distractor)
            )
            ...
            for edit_center in edit_centers_query:
                parts_in_rect = []
                part_rectangle = part_map_query[
                    0,
                    :,
                    edit_center[1] - stride // 2 : edit_center[1] + stride // 2,
                    edit_center[0] - stride // 2 : edit_center[0] + stride // 2,
                ]
            ...
            for edit_center in edit_centers_distractor:
                parts_in_rect = []
                part_rectangle = part_map_distractor[
                    0,
                    :,
                    edit_center[1] - stride // 2 : edit_center[1] + stride // 2,
                    edit_center[0] - stride // 2 : edit_center[0] + stride // 2,
    ...
    print(
        sum(nr_swaps_required) / len(nr_swaps_required)
        - sum(nr_swaps_reasonable) / len(nr_swaps_reasonable)
    )
