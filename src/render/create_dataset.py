...
def create_dataset_json(
    ...
    for c in range(len(classes)):
        ...
        for s in range(samples_per_class):
            ...
            ):  # randomly remove n parts from the bird to allow
                # interventions to be in domain
                if random.choice([0, 1]):
                    nr_delete = random.randint(0, len(part_keys))
                    part_keys_keep = delete_rand_items(part_keys, nr_delete)
                else:
                    part_keys_keep = part_keys  # removing from every sample parts
                    # reduces performance of trained networks quite a lot...
                    # so just remove parts from 50%
            else:
            ...
            for bg_part_i in range(nr_bg_parts):
                ...
                )  # TODO: 0-4 are the existing background part ids
                # ADJUST IF NEW PART IS ADDED
...
parser.add_argument(
    "--render_dataset", action="store_true", help="create_datasert_json"
)
...
if args.create_dataset_json:
    dataset_json = create_dataset_json(
        args.nr_samples_per_class, classes, parts, 0, 35, args.mode
    )
